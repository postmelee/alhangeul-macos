...
제보 샘플 `/Users/melee/Documents/projects/forks/rhwp/samples/hwp3-sample16.hwp`는 `HWP Document File V3.00` header를 가진 HWP 3.0 문서이며, 현재 고정된 `rhwp v0.7.16` core는 이 파일을 page 64개 문서로 parse/render할 수 있다. 이번 작업은 core가 처리할 수 있는 HWP 3.0 문서를 HostApp의 Swift validator와 bundled `rhwp-studio` URL byte guard가 사전에 차단하지 않도록 두 guard를 보강했다.
...
| `Sources/HostApp/Resources/rhwp-studio/assets/index-2nxfiXnQ.js` | bundled Studio minified asset의 URL byte kind guard에서 HWP5 CFB 또는 HWP 3.0 prefix이면 `hwp`로 분류하도록 보강했다. HWPX/HTML/unknown 판정 순서는 유지했다. |
...
| Studio asset 변경 | `index-2nxfiXnQ.js` minified 1줄 치환, 1 insertion/1 deletion |
...
| `node --check Sources/HostApp/Resources/rhwp-studio/assets/index-2nxfiXnQ.js` | OK | bundled Studio asset syntax check 통과 |
| `xcodebuild -project Alhangeul.xcodeproj -scheme HostApp -configuration Debug -derivedDataPath build.noindex/DerivedData CODE_SIGNING_ALLOWED=NO build` | OK | `** BUILD SUCCEEDED ** [0.429 sec]` |
...
| wasm asset count test | OK | `rhwp_bg-BuHc-de_.wasm` 1개 확인 |
| built app Studio guard 확인 | OK | built app resource의 `index-2nxfiXnQ.js`에 HWP 3.0 prefix byte array 포함 |
| `./scripts/render-debug-compare.sh /private/tmp/alhangeul-hwp3-sample16-task371 --page 1 /Users/melee/Documents/projects/forks/rhwp/samples/hwp3-sample16.hwp` | OK | render tree, core SVG, native PNG, diff PNG 생성 |
...
- `Sources/HostApp/Resources/rhwp-studio/assets/index-2nxfiXnQ.js`는 minified bundled asset이다. 향후 `rhwp-studio`를 upstream dist로 다시 동기화하면 이번 URL byte guard 보강이 사라질 수 있으므로, Studio sync 작업에서 HWP 3.0 guard를 다시 확인해야 한다.
...
최종 결과보고서 기준으로 PR 게시 단계 진행 승인을 요청한다. 승인 후 `publish/task371` 원격 브랜치로 push하고 `devel` 대상 PR을 생성한다.
