...
| 9 | 본 커밋 | 현재 매핑 가능한 파일 단축키를 HostApp 네이티브 브리지로 연결 |
...
Issue #134의 목표였던 MVP HostApp Viewer의 `rhwp-studio` WKWebView 기반 전환은 완료됐다. `devel-webview` 브랜치에서 build와 최소 HWP/HWPX open smoke가 통과했고, 수동 테스트에서 발견된 `rhwp-studio` CSS/JS 미적용, WASM fetch 실패, 파일 메뉴 명령과 파일 단축키 미동작 문제도 보정했다. Quick Look/Thumbnail native render 경로는 유지했다.
