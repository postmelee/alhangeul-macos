...
| 12 | `08b9414` | HostApp toolbar에 공유, Finder에서 보기, PDF로 내보내기, 최근 문서 기능 추가 |
| 13 | `6795f46` | 공유를 `exportHwp` 기반으로 전환하고 PDF snapshot 전 editor settle 처리 추가 |
| 14 | 본 커밋 | 공유 picker가 toolbar 공유 버튼 위치를 기준으로 뜨도록 anchor view 보정 |
...
- 공유는 HostApp 로드 시점 원본 bytes가 아니라 `rhwp-studio`의 현재 `exportHwp` 결과 bytes를 사용한다.
- 저장, 공유, 인쇄, PDF export는 실행 전 active editor element의 `change`/`blur`와 animation frame 대기를 거쳐 editor 입력 상태를 먼저 정리한다.
- 공유 picker는 window content view 임의 좌표가 아니라 SwiftUI toolbar 공유 버튼에 심은 AppKit anchor view를 기준으로 표시한다.
...
| `Sources/HostApp/Services/SharePresentationAnchor.swift` | SwiftUI toolbar 공유 버튼 위치를 AppKit picker anchor로 노출하는 `NSViewRepresentable` bridge |
| `Sources/HostApp/Services/RhwpStudioHostBridgeScript.swift` | 파일 메뉴/단축키/toolbar 명령 캡처, editor settle, `rhwp-request` export/print/PDF/share payload 생성 user script |
...
| `mydocs/working/task_m010_134_stage13.md` | Stage 13 공유/PDF 최신 상태 반영 보정 보고서 |
| `mydocs/working/task_m010_134_stage14.md` | Stage 14 공유 picker 위치 보정 보고서 |
...
공유 picker 위치 보정 후 확인:

```bash
xcodegen generate
```

결과: 성공. 새 `SharePresentationAnchor.swift` 파일이 Xcode project에 포함됨.
...
| 공유가 HostApp 로드 시점 원본 bytes 대신 `exportHwp` 결과를 사용 | OK |
| PDF snapshot 생성 전 editor focus/입력 상태 settle 처리 | OK |
| 공유 picker가 toolbar 공유 버튼 anchor 기준으로 표시 | OK |
...
- Stage 13의 editor settle은 WebKit focus/blur와 animation frame 기준의 최소 동기화다. IME 조합 중인 글자까지 강하게 보장하려면 upstream `rhwp-studio`에 explicit commit/flush API를 추가하는 후속 작업이 더 정확하다.
- Stage 14의 공유 picker 위치는 SwiftUI toolbar 내부 view 계층에 의존한다. toolbar display mode 또는 macOS 버전에 따라 anchor ancestor가 달라질 수 있으므로 수동 QA에서 위치를 확인해야 한다.
