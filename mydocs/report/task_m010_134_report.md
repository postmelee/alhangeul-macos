...
| 10 | 본 커밋 | HostApp viewer 하단 중복 footer 제거 |
...
| `Sources/HostApp/HostApp.swift` | native zoom command menu 제거, File menu 열기/저장/인쇄 command 연결 |
...
Issue #134의 목표였던 MVP HostApp Viewer의 `rhwp-studio` WKWebView 기반 전환은 완료됐다. `devel-webview` 브랜치에서 build와 최소 HWP/HWPX open smoke가 통과했고, 수동 테스트에서 발견된 `rhwp-studio` CSS/JS 미적용, WASM fetch 실패, 파일 메뉴 명령과 파일 단축키 미동작, HostApp 하단 중복 footer 문제도 보정했다. Quick Look/Thumbnail native render 경로는 유지했다.
