...
| 8 | 본 커밋 | `rhwp-studio` 파일 메뉴 열기/저장/인쇄를 HostApp 네이티브 기능으로 연결 |
...
| `Sources/HostApp/Services/RhwpStudioHostBridgeScript.swift` | 파일 메뉴 명령 캡처와 `rhwp-request` export/print payload 생성 user script |
...
- Stage 5 smoke는 process/window 유지 기준의 최소 검증이다. Stage 7에서 custom resource scheme 기반 렌더링과 toolbar/menu event listener 등록 경로는 확인했고, Stage 8에서 파일 메뉴 native bridge를 build 검증했다. 저장/인쇄 패널 표시와 실제 파일 출력은 실행 중인 앱에서 추가 수동 QA가 필요하다.
...
Issue #134의 목표였던 MVP HostApp Viewer의 `rhwp-studio` WKWebView 기반 전환은 완료됐다. `devel-webview` 브랜치에서 build와 최소 HWP/HWPX open smoke가 통과했고, 수동 테스트에서 발견된 `rhwp-studio` CSS/JS 미적용, WASM fetch 실패, 파일 메뉴 명령 미동작 문제도 보정했다. Quick Look/Thumbnail native render 경로는 유지했다.
