...
| 6 | 본 커밋 | 수동 테스트에서 확인된 `rhwp-studio` CSS/JS 미적용 문제 보정 |
...
- Stage 5 smoke는 process/window 유지 기준의 최소 검증이다. Stage 6에서 CSS/JS가 적용된 `rhwp-studio` shell은 확인했지만, WKWebView 내부 렌더링의 픽셀/텍스트 정합성, 페이지 이동, zoom UI 세부 동작은 추가 수동 QA 또는 UI 자동화가 필요하다.
...
Issue #134의 목표였던 MVP HostApp Viewer의 `rhwp-studio` WKWebView 기반 전환은 완료됐다. `devel-webview` 브랜치에서 build와 최소 HWP/HWPX open smoke가 통과했고, 수동 테스트에서 발견된 `rhwp-studio` CSS/JS 미적용 문제도 보정했다. Quick Look/Thumbnail native render 경로는 유지했다.
