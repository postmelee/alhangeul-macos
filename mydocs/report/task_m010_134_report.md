...
| 11 | 본 커밋 | HostApp 상단과 File menu의 중복 문서 열기 제거 |
...
| `Sources/HostApp/Views/RhwpStudioWebView.swift` | `WKWebView` wrapper, resource/document scheme registration, navigation policy, revision 기반 reload, AppKit key equivalent fallback |
...
| `Sources/HostApp/Views/ContentView.swift` | HostApp titlebar toolbar의 중복 `문서 열기` 버튼 제거 |
...
- Stage 5 smoke는 process/window 유지 기준의 최소 검증이다. Stage 7에서 custom resource scheme 기반 렌더링과 toolbar/menu event listener 등록 경로는 확인했고, Stage 8과 Stage 9에서 파일 메뉴와 파일 단축키 native bridge를 build 및 panel/window smoke로 검증했다. 실제 저장 파일 내용과 인쇄 출력물의 시각적 검수는 별도 QA가 필요하다.
...
Issue #134의 목표였던 MVP HostApp Viewer의 `rhwp-studio` WKWebView 기반 전환은 완료됐다. `devel-webview` 브랜치에서 build와 최소 HWP/HWPX open smoke가 통과했고, 수동 테스트에서 발견된 `rhwp-studio` CSS/JS 미적용, WASM fetch 실패, 파일 메뉴 명령과 파일 단축키 미동작, HostApp 하단 중복 footer와 상단 중복 열기 문제도 보정했다. Quick Look/Thumbnail native render 경로는 유지했다.
