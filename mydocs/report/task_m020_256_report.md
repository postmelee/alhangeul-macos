...
- Copilot review follow-up으로 HWP embedded thumbnail 경로도 `backendUsed: .embeddedThumbnail` diagnostics를 명시해 ImageIO thumbnail decode가 CoreGraphics render로 기록되지 않게 보정했다.
...
| backend | 의미 |
|---|---|
| `.coreGraphics` | render tree + `CGTreeRenderer` 경로로 생성한 이미지 |
| `.skia` | `rhwp_render_page_png` 결과 PNG를 ImageIO로 decode한 이미지 |
| `.embeddedThumbnail` | HWP embedded thumbnail bytes를 ImageIO로 decode한 이미지 |

`embeddedThumbnail` 경로는 문서 본문을 render하지 않으므로 `pageSize`는 embedded thumbnail metadata의 width/height, `pixelSize`는 실제 반환 `CGImage` pixel size를 기록한다. 이 경로는 Skia PNG render 결과가 아니므로 `pngBytes`는 nil이고, `durationMs.totalMs`에는 ImageIO decode 시간을 기록한다.
...
Stage 5 및 Copilot review follow-up에서 다음 최종 검증을 다시 실행했다.

```bash
./scripts/check-no-appkit.sh
rg -n "coreGraphicsOnly|skiaOptIn|embeddedThumbnail|backendUsed|fallbackReason|pngBytes|durationMs|#257|#258" \
...
| `rg -n "coreGraphicsOnly\|skiaOptIn\|embeddedThumbnail\|backendUsed\|fallbackReason\|pngBytes\|durationMs\|#257\|#258" ...` | 통과: source, 최종 보고서, 오늘할일 연결 확인 |
| `xcodebuild ... -scheme HostApp ... build` | 통과: `** BUILD SUCCEEDED ** [4.756 sec]` |
| `xcodebuild ... -scheme QLExtension ... build` | 통과: `** BUILD SUCCEEDED ** [0.314 sec]` |
| `xcodebuild ... -scheme ThumbnailExtension ... build` | 통과: `** BUILD SUCCEEDED ** [0.426 sec]` |
| `git diff --check` | 통과 |
...
- Quick Look logging은 `page.diagnostics.backendUsed`, `fallbackReason`, `pngBytes`, `durationMs`를 기록하면 된다. 작은 Finder thumbnail 요청에서 embedded thumbnail이 쓰이면 `backendUsed`는 `.embeddedThumbnail`으로 구분된다.
...
- Thumbnail logging/cache value는 `HwpRenderedPage.diagnostics`를 그대로 읽을 수 있다. Embedded thumbnail 경로는 `.embeddedThumbnail` backend와 `pngBytes: nil`로 Skia PNG render 결과와 구분된다.
...
- Copilot review comment에서 지적된 embedded thumbnail diagnostics 문제는 `.embeddedThumbnail` backend 추가와 명시 diagnostics 구성으로 보정했다.
...
#256은 Shared renderer에 Skia optional backend와 CoreGraphics fallback contract를 구현했다. 기본 제품 동작은 `coreGraphicsOnly`로 유지했고, `skiaOptIn`을 명시한 경우에만 Skia 우선 + CoreGraphics fallback이 동작한다. Embedded thumbnail은 별도 backend diagnostics로 구분해 후속 logging/cache 작업이 CoreGraphics render와 혼동하지 않도록 했다.
