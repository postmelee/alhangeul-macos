...
**공개 최초 설치 수용 기준은 아직 미완료다.** Stage 5에서 importer가 발견된 뒤 기존 문서의 재색인을 앱이 자동 요청하도록 구현했다. 발견 전에 보낸 요청은 종료 코드 0이어도 기존 문서가 검색되지 않는 순서 문제를 확인했다. 실제 제품의 재실행 이후 세 형식 본문 및 한글 검색과 앱 종료 상태의 자동 수정·보호·삭제 전파가 통과했다. 최종 후보의 새 경로 첫 실행 검증은 진행 중이다.
