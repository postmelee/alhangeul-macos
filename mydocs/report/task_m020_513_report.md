...
## 현재 판정 — 2026-09-13 릴리즈 VM 재설치 수용 연결

PR #530 은 devel `010b01b`에 병합됐다. 후속 Stage 11~13에서 새 VM 최초 설치 뒤 같은 후보의 제거·재설치와 종료 후 검색을 자동 실행하고, 재설치 증거 없이 공개 승격할 수 없도록 연결했다. **구현·회귀 검증 완료이며 새 서명 후보의 VM 수용 완료는 아니다.**

- 후보 sandbox는 bundle ID/container metadata와 요청 키의 importerPath로 찾는다. 요청 키만 읽고 설정 초기화·전체 preferences 복사는 하지 않는다. 누락·다른 앱·모호한 기록은 실패 처리한다.
- 최초/재설치 snapshot을 분리하고 후보 bytes·경로·빌드·mtime·원본 corpus, 설치 객체 변화·요청 식별자 변화·단일 재설치 실행·검색/종료 후 검색을 대조한다. 최초 종료/검색 증거를 재설치 결과로 재사용하거나 검색 뒤 설치 객체/요청 기록이 바뀌면 거부한다.
- 승격 결과 schema 2와 재설치 JSON 두 개를 필수로 요구한다. 구형 최초 설치 전용 PASS는 현재 gate의 근거가 아니다. 기존 공개 v0.2.0 기록/tag/DMG는 유지한다.
- 설치 회귀 **20개**, 승격 회귀 **15개**, 표준 Spotlight 운영 회귀 **36개**, 전체 workflow actionlint 및 diff 검사 PASS. 새 제품 앱 빌드·서명·공증·실제 설치/색인·공개 배포는 수행하지 않았다.

[Stage 11](../working/task_m020_513_stage11.md), [Stage 12](../working/task_m020_513_stage12.md), [Stage 13](../working/task_m020_513_stage13.md)에 검증과 후속 범위를 기록했다. workflow CI 결과는 PR Checks/완료 코멘트에서 최종 head 기준으로 확인한다. UI 변경이 없으므로 이전 Spotlight 캡처를 새 자동화 실행 증거로 첨부하지 않는다.

다음은 이 보완 PR 리뷰·병합 후 별도 릴리즈 실행 이슈에서 새 패치 버전/빌드와 후보 SHA를 확정하는 순서다. 현재 공개 v0.2.0(18)을 기준으로 **v0.2.1(19)** 을 제안하며 아직 버전 파일/tag를 변경하지 않았다. 같은 서명·공증 DMG로 양 VM 최초 설치·동일 버전 재설치·실제 Mac 수용을 확인하고, 공개 후 0.2.0 → 새 버전 Sparkle를 검증한다. #513 / #337 은 해당 수용까지 열린 상태로 유지한다. macOS 12 환경 부재와 #525 는 별도 한계로 판단한다.

## Stage 8~10 판정 — 동일 버전 재설치 제품 보완

**새 개발 후보에서 기존 요청 기록을 유지한 동일 버전 재설치의 자동 본문 검색·앱 종료 후 검색이 통과했다.** 새 설치 디렉터리의 볼륨·inode·생성 시각을 접수 기록에 추가해, 같은 경로·버전·mtime의 재설치도 과거 설치와 구분한다. 일반 앱 재실행은 중복 요청을 생략하고 구형 기록은 요청 성공 한 번으로 전환한다. 실패 시 이전 기록을 보존한다.

- Swift 20개, 운영 smoke 36개, 기존 release install 11개·bundle 5개 회귀 PASS. Debug/universal Release ad-hoc 빌드 및 importer callback/번들 계약 PASS.
- 현재 Mac의 공개 앱을 잠시 보존·격리한 첫 후보는 118.18초에 본문 검색/후보 선택 PASS. 요청 키를 유지한 같은 버전 제거·복사 뒤에는 **42.09초**에 영문 3개·한글 2개 검색 PASS였고, 앱 종료 후에도 유지됐다.
- 새 설치 전 문서 미검색, 같은 경로/빌드/mtime, 새 설치 객체와 새 요청 식별자, corpus hash/mtime 보존을 확인했다. 후보 시험 중 외부 등록/색인·touch·요청 키 초기화·문서 열기는 하지 않았다.
- 처음 공개 앱과 공존한 시험은 사전 본문 검색으로 제외했다. 정리 시 카탈로그 지연과 후보 전용 창 위치 키 잔존을 보정하고 공개 앱의 전체 bytes·설정 dictionary·provider, 소유 파일/검색 결과/카탈로그 제거를 최종 확인했다.

[Stage 8](../working/task_m020_513_stage8.md), [Stage 9](../working/task_m020_513_stage9.md), [정제 실제 결과](assets/task_m020_513/same-version-reinstall-results.json)에 구현·검증·한계를 구분했다. 실제 재설치의 새 GUI 캡처는 없으며 이전 스크린샷으로 대체하지 않는다.

공개 v0.2.0의 새 VM 최초 설치와 실제 Sparkle 검증은 [#520 Stage 5](../working/task_m900_520_stage5.md)에서 완료됐다. **이번 수정은 공개 v0.2.0 DMG에 포함되지 않는다.** 후속 PR 리뷰/병합 뒤 새 서명·공증 후보의 설치·재설치·Sparkle 검증이 필요하며 #513 / #337 은 열린 상태로 유지한다. macOS 12 실행·Intel 실기기 검증도 미실행이다. 아래 Stage 7 이하의 공개 전 관문 표기는 당시 기록이다.
