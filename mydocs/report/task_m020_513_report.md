...
**공개 최초 설치 수용 기준은 아직 미완료다.** Stage 5에서 importer가 발견된 뒤 기존 문서의 재색인을 앱이 자동 요청하도록 구현했다. 발견 전에 보낸 요청은 종료 코드 0이어도 기존 문서가 검색되지 않는 순서 문제를 확인했다. 실제 제품의 재실행 이후 세 형식 본문 및 한글 검색과 앱 종료 상태의 자동 수정·보호·삭제 전파가 통과했다. 최종 후보는 현재 Mac의 새 경로에 복사한 뒤 한 번 실행하여 자동 발견·설치 전 문서 본문 검색·실제 후보 선택과 앱 종료 후 검색까지 통과했다. 발견까지 약 7분 58초가 걸렸다.

[Stage 5](../working/task_m020_513_stage5.md)와 [합성 결과 JSON](assets/task_m020_513/launch-reindex-results.json)에 최신 구현·실행 결과를 기록한다. 아래 Stage 1–4 기록은 조사 PR #514까지의 과거 결과이며, 새 코드의 검증 결과와 구분한다. #513 및 상위 #337은 열린 상태를 유지한다. 별도 초기 계정/VM, macOS 12/Intel 실행 환경은 없으며 서명·공증 공개 후보는 별도 릴리스 관문이다.
