...
| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| `project-artifact-cleanup` Skill | 없음 | `mydocs/skills/project-artifact-cleanup/SKILL.md` 215 lines |
...
| 분류 | 대표 예 |
|------|---------|
| `safe` | `build.noindex/DerivedData*`, `output/stage3-render*`, `output/task*-*`, `/private/tmp/rhwp-task*-render`, `/private/tmp/rhwp-task*-swift-module-cache`, `/private/tmp/rhwp-task*-clang-module-cache`, `/private/tmp/task*-pr-body.md`, `/private/tmp/alhangeul-*` |
| `approval-required` | `build.noindex/release`, `RustBridge/target`, `Frameworks`, `/private/tmp/rhwp-core-*`, 상태가 불분명한 현재 task 산출물, 이전 이름 설치본 충돌 후보 |
| `never-delete` | 저장소 루트, `/private/tmp` 자체, `$HOME`, `$HOME/Applications/AlhangeulMac.app`, git worktree, `.git` 보유 경로 |

### Debug build cleanup

Debug build는 compile/link 확인용이므로 `build.noindex/DerivedData*` 정리 후보가 될 수 있다. 다만 Quick Look/Thumbnail/Viewer 테스트가 계속 필요하면 먼저 Release package 산출물을 `$HOME/Applications/AlhangeulMac.app` 표준 설치본으로 갱신하고 `lsregister`, `pluginkit`, `qlmanage -r`, `qlmanage -r cache`, `/tmp/alhangeul-ql` 생성, `qlmanage -t` 확인을 거친 뒤 Debug 산출물 삭제를 제안하도록 했다.
...
## 단계별/PR 리뷰 반영 커밋 히스토리

```text
5deda73 Task #130: PR 리뷰 후속 지적 반영
fbe8730 Potential fix for pull request finding
2d24450 Potential fix for pull request finding
4f87cc3 Task #130: PR 리뷰 검증 기록 보강
b53a392 Task #130: PR 리뷰 추가 지적 반영
eb347aa Task #130: PR 리뷰 지적 반영
b16d649 Potential fix for pull request finding
...
위 목록은 PR #131의 기존 단계/리뷰 반영 커밋이다. 이후 후속 리뷰 반영 커밋은 PR commit log를 기준으로 확인한다.
