...
| 분류 | 대표 예 |
|------|---------|
| `safe` | `build.noindex/DerivedData*`, `output/task*-*`, `/private/tmp/rhwp-task*-stage*`, `/private/tmp/task*-pr-body.md`, `/private/tmp/alhangeul-*` |
...
| `rg -n "project-artifact-cleanup|dry-run|never-delete|approval-required" mydocs/skills/project-artifact-cleanup/SKILL.md` | OK |
| `git diff --check` | OK |
...
본 보고 커밋이 추가된다: `Task #130 Stage 3 + 최종 보고서: 부산물 정리 Skill 검증과 보고`.
...
- 오늘할일 완료 처리 완료
- 최종 보고서 작성 완료
- PR 생성은 작업지시자 승인 후 `task-final-report` 절차로 진행한다.

## 작업지시자 승인 요청

본 최종 보고서 검토 후 PR 게시 단계(`publish/task130` push + `devel` 대상 PR 생성) 진행 승인을 요청한다.
