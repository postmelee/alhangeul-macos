...
`ios/devel`에서 사용하던 native viewer core 변경을 `edwardkim/rhwp`의 `devel`에 선별 반영하고, `alhangeul-macos`가 해당 core commit을 submodule로 추적하도록 전환했다.

이번 작업은 `alhangeul-macos` 저장소의 Issue #3과 PR에서 추적한다.

## 주요 변경

- `Vendor/rhwp` submodule URL을 `https://github.com/edwardkim/rhwp.git`로 변경했다.
- `rhwp-core.lock`을 `edwardkim/rhwp` commit `1e9d78a1d40c71779d81c6ec6870cd301d912626`로 갱신했다.
- `edwardkim/rhwp` core에 native viewer용 최소 API를 추가했다.
...
- README와 architecture 문서를 core 기준으로 갱신했다.
...
- 향후 core 고도화는 `edwardkim/rhwp`의 `devel`에서 진행하고, 앱 연동 변경은 `alhangeul-macos` Issue/PR로 추적한다.
