...
2026-09-15 최신 통합 상태: v0.2.2까지 화면 보정을 확장하고 실제 Release·앱 feed·Homebrew를 v0.2.2로 정렬했다. 작업지시자가 PR #527 갱신·devel 병합·별도 Pages 공개 반영까지 승인했다. [Stage 4 통합·검증 기록](../working/task_m900_523_stage4.md)이 최신 결과이며, 아래 v0.2.0 조회·승인 대기 문구는 9월 13일 당시 기록이다.

업데이트 페이지의 소개·공개일·버전 위계를 정리하고, Homebrew 설치와 업데이트 명령을 탭으로 전환하도록 개선했다. Homebrew 영역 높이는 데스크톱·모바일에서 약 29% 줄었으며 선택한 명령을 복사할 수 있다. 구현과 로컬 검증을 완료했고, 최종 보고 승인 후 v0.2.0 공개 상태까지 정렬해 [리뷰 PR #527](https://github.com/postmelee/alhangeul-macos/pull/527)을 게시했다. 공개 배포 완료 보고는 아니다.
...
1. `publish/task523` → `devel` 대상 [Open PR #527](https://github.com/postmelee/alhangeul-macos/pull/527)을 생성했다. 최종 head의 PR CI 결과는 PR 본문에 기록한다. 이번 승인은 리뷰 PR 게시 범위이며 merge는 별도다.
