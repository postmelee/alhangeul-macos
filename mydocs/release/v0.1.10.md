...
`v0.1.10`은 평문 HWP5/HWPX 형식별 native 저장과 재열기, 현재 편집 상태의 전체 페이지 PDF·인쇄, 해당 offscreen renderer의 WebKit 보안 경계와 `rhwp v0.8.4` 문서 호환성 개선을 묶는 patch release다. upstream core/studio의 암호 저장 지원이 알한글 native 저장 경로의 암호 저장 지원을 뜻하지는 않는다.

- 평문 HWP5와 HWPX를 선택한 형식 그대로 저장하고 저장 결과를 다시 열 수 있다.
- HWP3는 열 수 있지만 원형 저장은 지원하지 않으며, 원본을 보존하는 새 HWP5/HWPX 변환 복사본으로만 저장한다.
- 암호 문서는 열 수 있지만 native 저장에서는 보호를 유지하지 않으며, 별도 사용자 확인 뒤 원본과 다른 새 경로에 평문 복사본만 저장한다.
...
- [#482](https://github.com/postmelee/alhangeul-macos/issues/482) 기준으로 HWP3는 열 수 있지만 HWP3 원형 저장은 지원하지 않는다. 평문 HWP3 저장은 형식 변환을 알린 뒤 원본·기존 파일과 다른 신규 경로에 HWP5/HWPX 변환 복사본만 기록한다.
- [#480](https://github.com/postmelee/alhangeul-macos/issues/480) 기준으로 암호 문서는 열 수 있지만 native 저장 경로가 암호 보호 유지나 새 암호 설정을 지원하지 않는다. 사용자 확인 뒤 원본과 다른 신규 경로에 평문 복사본만 기록한다.
