...
`v0.1.10`은 HWP/HWPX 형식별 native 저장과 재열기, 현재 편집 상태의 전체 페이지 PDF·인쇄, 해당 offscreen renderer의 WebKit 보안 경계와 `rhwp v0.8.4` 문서 호환성 개선을 묶는 patch release다. upstream core/studio의 암호 저장 지원이 알한글 native 저장 경로의 암호 저장 지원을 뜻하지는 않는다.

- HWP와 HWPX를 선택한 형식으로 저장하고 저장 결과를 다시 열 수 있다.
...
배포 후 저장 안전성 검토에서 다음 두 문제가 확인됐다. 두 수정은 `v0.1.10` tag에 포함되지 않았으며 다음 릴리스 후보에서 반영한다.

- [#482](https://github.com/postmelee/alhangeul-macos/issues/482): 평문 HWP3의 native 저장이 형식 변환 안내 없이 HWP5 bytes로 원본을 덮어쓸 수 있다.
- [#480](https://github.com/postmelee/alhangeul-macos/issues/480): 보호 문서의 native 저장이 보호 해제 안내 없이 평문 bytes로 원본을 덮어쓸 수 있다.
- 수정 릴리스 전에는 HWP3·암호 문서를 먼저 복사하고 `v0.1.10`의 native 저장을 사용하지 않는다.
...
| [#457](https://github.com/postmelee/alhangeul-macos/pull/457) | Task #456: HostApp HWP/HWPX native 저장 경로 연결 | 사용자-facing 기능 | 예 | 예 | [#456](https://github.com/postmelee/alhangeul-macos/issues/456) | 없음 | PR body, `mydocs/report/task_m010_456_report.md` | 평문 HWP5/HWPX 형식별 저장·재열기, HWP3·암호 저장 제한은 #480/#482에서 보정 |
...
- 평문 HWP5와 HWPX를 형식에 맞는 macOS 저장 패널로 저장하고, 저장 결과를 같은 형식으로 다시 열 수 있습니다.
...
- upstream core/studio는 HWP5 EncryptVersion 4, HWP3와 암호화 HWPX의 열기·암호화 저장·재열기 계약을 확장했습니다. 알한글 native 저장 경로는 이 암호 저장 API를 아직 연결하지 않았습니다.
...
- 평문 HWP3는 저장 전 형식 변환을 알리고, 원본이나 다른 기존 파일을 덮어쓰지 않는 새 HWP5/HWPX 변환 복사본으로만 저장합니다.
- 암호 문서는 열 수 있지만 native 저장에서는 보호를 유지하지 않으며, 확인 뒤 원본과 다른 새 경로의 평문 복사본으로만 저장합니다.
...
- [#482](https://github.com/postmelee/alhangeul-macos/issues/482): `v0.1.10` 배포본은 HWP3 원형 저장을 지원하지 않고, 평문 HWP3 저장 시 HWP5 bytes로 원본을 덮어쓸 수 있다. PR #483에서 변환 안내와 신규 destination 전용 저장으로 보정한다.
- [#480](https://github.com/postmelee/alhangeul-macos/issues/480): `v0.1.10` 배포본의 native 저장은 암호 보호 유지나 새 암호 설정을 지원하지 않고, 보호 문서 원본을 평문으로 덮어쓸 수 있다. PR #481에서 사용자 확인과 신규 평문 복사본 전용 저장으로 보정했다.
- 평문 HWP5/HWPX 저장은 대표 fixture의 container, text/nonblank render와 재열기를 확인했지만 upstream exporter의 모든 요소에 대한 의미론적 완전 무손실을 보장하지 않는다.
...
- [x] upstream 암호 저장 지원과 알한글 native 저장 경계를 분리하고 HWP3 변환 저장 제한을 설명
