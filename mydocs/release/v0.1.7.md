...
| 항목 | 값 |
|------|----|
| 상태 | `v0.1.7` build `13` Stage 2 완료, Stage 3 source preflight 승인 대기 |
| 목표 Git tag | `v0.1.7` |
...
| Debug build | `HostApp` Debug, `CODE_SIGNING_ALLOWED=NO` | Stage 3 예정 |
| Native render smoke | `KTX.hwp`, `request.hwp`, `exam_kor.hwp` 첫 페이지 렌더 | Stage 3 예정 |
| Release bundle | local unsigned Release app/extension universal slice | Stage 3 예정 |
...
| Delta checklist | `scripts/ci/write-release-delta-checklist.sh v0.1.6 HEAD ...` | Stage 2/3 확인 |
| Local rehearsal DMG | `./scripts/release.sh --skip-notarize 0.1.7` | Stage 3 예정 |
