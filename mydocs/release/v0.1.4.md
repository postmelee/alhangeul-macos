...
| 항목 | 값 |
|------|----|
| 상태 | `v0.1.4` build `10` source metadata와 release communication 준비, source preflight 승인 대기 |
| 목표 Git tag | `v0.1.4` |
...
| Local rehearsal DMG | Stage 3 이후 생성 예정 |
| Local rehearsal DMG SHA256 | Stage 3 이후 기록 예정 |
...
| Release helper | `scripts/ci/write-release-notes.sh 0.1.4 ...` template check | Stage 2에서 확인 |
| Pages release note | `docs/updates/v0.1.4.html` | Stage 2에서 확인 |
| Delta checklist | `scripts/ci/write-release-delta-checklist.sh v0.1.3 HEAD ...` | Stage 2에서 초안 확인, Stage 3에서 최종 HEAD 기준 재실행 |

Stage 3 이후 보강할 검증:

- source preflight와 local/rehearsal DMG 생성
