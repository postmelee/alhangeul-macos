...
- HostApp titlebar toolbar는 macOS 공유, Finder에서 보기, PDF로 내보내기, 최근 문서 접근을 제공한다.
...
11. `파일 > 인쇄`는 `rhwp-studio`의 page SVG response를 별도 WKWebView에 싣고 AppKit print operation을 실행한다.
12. `PDF로 내보내기`는 같은 page SVG response를 PDF save job으로 실행하고 저장 완료 후 Finder에서 결과 파일을 표시한다.
13. `공유`는 현재 HostApp이 보관한 문서 bytes를 임시 파일로 만든 뒤 `NSSharingServicePicker`로 전달한다.
