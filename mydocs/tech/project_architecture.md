...
- `Shared/HwpPreviewPDFRenderer`와 `Shared/HwpPageImageRenderer`를 사용해 render tree 기반 page bitmap을 생성하고 Quick Look 표시용 PDF preview로 반환한다.
...
- 파일 크기 제한, page index 기반 render tree 요청, bitmap context 생성, PNG 인코딩, Quick Look 표시용 PDF preview 생성을 공통 처리한다.
...
2. `HwpPreviewProvider`가 `Shared/HwpPreviewPDFRenderer`에 preview 생성을 요청한다.
3. `HwpPreviewPDFRenderer`가 `RhwpDocument`를 열고 `pageCount`만큼 page index를 순회한다.
4. 각 페이지는 `Shared/HwpPageImageRenderer`가 render tree 기반 bitmap으로 그린다.
5. preview는 페이지별 bitmap을 Quick Look 표시용 PDF page에 담아 `.pdf` `QLPreviewReply`로 반환한다.
6. 파일이 50 MB를 초과하면 텍스트 fallback을 반환한다.

이 PDF는 Finder Quick Look이 다중 페이지 preview를 표시하도록 하는 임시 컨테이너이며, 사용자용 PDF export나 HWP/HWPX 구조 변환 산출물이 아니다.
...
- Quick Look preview는 렌더된 page bitmap들을 PDF page에 삽입해 반환한다.
