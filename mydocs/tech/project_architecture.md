...
- 현재 lock은 `v0.7.8` Stable release tag pin 상태다. `rhwp-core.lock`은 release tag `v0.7.8`과 resolved commit `42cf91b6ba7b50fa1c853c01158a52ef68b45442`를 함께 기록한다.
- `v0.7.8`에는 현재 `RustBridge`가 사용하는 `build_page_render_tree`, `get_bin_data`, `render_page_svg_native`, `get_page_info_native`, `extract_thumbnail_only` API가 포함되어 있다.
