...
- 앱 bundle의 `Resources/rhwp-studio/index.html`을 `WKWebView`로 열고, `alhangeul-document://current` 내부 scheme으로 현재 문서 bytes를 제공한다.
- HostApp MVP viewer의 zoom/page/search 조작은 `rhwp-studio` 내부 UI가 소유한다.
- `project.yml` 기준으로 `Sources/Shared`, `Sources/RhwpCoreBridge`, `Frameworks/Rhwp.xcframework`를 포함하지만, MVP viewer 화면은 native render tree 경로를 호출하지 않는다.
...
- HostApp MVP viewer는 `rhwp-studio` WKWebView 경로를 사용하므로 이 native bitmap helper를 직접 호출하지 않는다.
...
- 현재 lock은 `v0.7.9` Stable release tag pin 상태다. `rhwp-core.lock`은 release tag `v0.7.9`와 resolved commit `0fb3e6758b8ad11d2f3c3849c83b914684e83863`을 함께 기록한다.
- `v0.7.9`에는 현재 `RustBridge`가 사용하는 `build_page_render_tree`, `get_bin_data`, `render_page_svg_native`, `get_page_info_native`, `extract_thumbnail_only` API가 포함되어 있다.
...
4. `RhwpStudioResourceLocator`가 app bundle의 `Resources/rhwp-studio/index.html`에 `url=alhangeul-document://current?revision=...`와 `filename=...` query를 붙여 entrypoint URL을 만든다.
5. `WKWebView`가 bundle의 `rhwp-studio` 정적 asset을 `loadFileURL(_:allowingReadAccessTo:)`로 연다.
6. `rhwp-studio`의 `loadFromUrlParam()`이 내부 document URL을 `fetch`하면 `RhwpStudioDocumentSchemeHandler`가 현재 revision의 bytes를 `application/octet-stream`으로 응답한다.
7. HWP/HWPX parsing, page layout, zoom/page UI는 `rhwp-studio`와 그 WASM bundle이 담당한다.

### Quick Look preview 경로

1. Finder가 `QLExtension`의 `HwpPreviewProvider`를 호출한다.
2. `HwpPreviewProvider`가 `Shared/HwpPreviewPDFRenderer.inspect(fileURL:)`로 파일 크기, page count, 첫 페이지 크기를 먼저 확인한다.
3. 단일 페이지 문서는 data creation block에서 첫 페이지 bitmap을 PNG로 encoding해 `.png` `QLPreviewReply`로 반환한다.
4. 다중 페이지 문서는 data creation block에서 `HwpPreviewPDFRenderer`가 `RhwpDocument`를 열고 `pageCount`만큼 page index를 순회한다.
