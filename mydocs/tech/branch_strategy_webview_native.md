# 제품 devel 승격과 native Skia 전환 브랜치 전략
...
이 문서는 `alhangeul-macos` 저장소의 제품 개발 브랜치와 native macOS shell/Skia editor integration 브랜치 전략을 정리한다. HostApp viewer/editor 첫 출시는 native renderer/editor 대신 `rhwp-studio`를 WKWebView로 로드하는 방식으로 진행되었고, 첫 public release 이후 제품 개발 라인을 `devel`로 승격하며 native 전환 라인은 별도 이름으로 보존한다.
...
- `native-viewer-editor`는 Swift native macOS shell, Rust/rhwp Skia renderer 연동, Swift 편집 UI/오버레이 장기 작업 통합 브랜치다.
...
| `devel` | 일반 제품 개발과 외부 기여 기본 브랜치. WKWebView-backed viewer/editor, Finder/Quick Look/Thumbnail, PDF/공유/저장, Mac 통합, 변환, 배포, 문서, Skia 공통 기반 작업을 받는다 | `devel-webview` 제품 라인과 `main`의 release 후속 기록을 통합한 commit으로 교체 |
| `native-viewer-editor` | 기존 `devel` head를 보존해 시작한 native macOS shell/editor integration 장기 개발 브랜치 | 기존 `origin/devel` head에서 새 원격 브랜치로 생성 |
...
기존 `devel`은 native viewer 렌더링 검증뿐 아니라 editor foundation 장기 작업까지 포함하는 라인이었다. 따라서 보존 브랜치 이름은 `native-viewer-editor`로 한다. #263 이후 이 이름은 Swift가 renderer 전체를 재구현한다는 뜻이 아니라 HostApp native macOS shell, Rust/rhwp Skia renderer, Swift 편집 UI/오버레이를 통합하는 장기 라인으로 해석한다.
...
`origin/devel` 전용 commit은 Task #119 font fallback과 Task #123 native renderer/body overflow replay/clip 정책 중심이다. 이 내용은 native 전환 브랜치 성격과 맞다.
...
| `devel` | 일반 제품 개발과 외부 기여 기본 브랜치. WKWebView-backed viewer/editor, Finder/Quick Look/Thumbnail, PDF/공유/저장, Mac 통합, 변환, 배포, 문서, Skia 공통 기반 작업 통합 | 일반 작업 PR의 기본 base |
| `native-viewer-editor` | HostApp native macOS viewer/editor shell, Rust/rhwp Skia renderer 연동, Swift 편집 UI/오버레이와 editor command routing 장기 개발 통합 브랜치 | HostApp native shell/overlay 작업 PR base |
...
2. 당시 native renderer/editor 동작을 직접 바꾸는 작업만 `devel`을 기준으로 시작한다.
3. release-critical 수정은 먼저 `devel-webview`로 반영한다.
4. 같은 수정이 native 전환 장기 브랜치에도 필요하면 별도 PR 또는 cherry-pick으로 `devel`에 후속 반영한다.
...
- WKWebView 기반 출시 라인과 native 전환 라인이 계속 분리된다.
...
- native macOS shell/editor integration 라인을 독립 이름으로 유지할 필요가 명확할 때
...
- [ ] `native-viewer-editor` 전용 HostApp native shell/overlay/editor integration commit을 release PR에 포함하지 않는다는 점을 확인했다.
...
- `native-viewer-editor` 보호 규칙: HostApp native shell/overlay/editor integration PR이 일반 출시 라인으로 섞이지 않게 한다.
...
- PR template/review instruction: 일반 작업 PR target은 `devel`, HostApp native shell/overlay 작업 PR target은 `native-viewer-editor`로 안내한다.
