...
후속 #30에서 `Vendor/rhwp`를 제거한 뒤에는 `scripts/update-rhwp-core.sh`를 submodule update script가 아니라 core dependency update gate로 재정의한다.
...
`scripts/build-rust-macos.sh`는 git dependency 전환 후 다음 기준으로 lock을 검증한다.
...
현재 #55 단계에서는 이 architecture를 문서화만 한다. 실제 script 재정의는 `Vendor/rhwp` 제거와 `RustBridge/Cargo.toml` 전환을 함께 수행하는 #30에서 처리한다.
