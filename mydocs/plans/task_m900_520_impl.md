...
### Stage 4.2 — PR 검토와 반복 릴리스 수용 기준 보강

2026-09-13 작업지시자의 진행 승인에 따라 #524의 draft 조회·후보/도구 분리·공개 직전 재검사를 검토한다. runbook에 최초 설치와 실제 Sparkle 업데이트를 서로 대체할 수 없는 필수 경로로 명시하고, 공개 다운로드 동일성 및 버전/검색/확장 결과 기록을 요구한다. Spotlight 첫 실행 대기 안내를 내부 기능 안내와 릴리스 본문 후보에 보강한다. 공개 Pages 문구 수정은 동일 후보 승격 후 별도 문서 반영으로 분리한다. 저장하지 않음 후 복구 후보 잔존은 소유한 시험 문서에서 재확인하고 출시 판단 근거를 남긴다. 후보 앱·tag·DMG는 변경하지 않는다.
