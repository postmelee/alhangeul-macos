...
### Stage 5.1 — 공개 사실과 수용 결과 기록

2026-09-13 공개 승인에 따라 동일 DMG 승격과 Pages/appcast 배포를 실행했다. 실제 Sparkle 설치·자동 재실행·확장은 통과했으나 기존 합성 문서의 자동 색인 누락이 관찰됐다. 공개 사실·새 문서 검색 성공·기존 문서 누락을 각각 기록하고 README/지원 안내를 실제 공개 상태로 정렬한다. 문서 검증을 통과한 진행 기록 PR로 제출하며 전체 릴리스 완료나 Stage 5 수용 완료로 표시하지 않는다.

### Stage 5.2 — 기존 문서 누락의 원인 분리와 최종 수용

업데이트 전부터 있던 원본을 보존하고 업데이트 후 새 사본과 비교한다. 진단용 추출 성공·앱에서 연 뒤 검색 성공을 첫 실행 자동 색인으로 대체하지 않는다. 현재 Mac은 동일 0.2.0 후보를 먼저 설치했던 이력이 있으므로 이전 앱만 복원한 시험과 일반 0.1.11 사용자 상태를 구분한다. 설정 조회가 불가능한 경우 요청 생략을 확정 원인으로 쓰지 않는다. #513 에서 남은 원인을 분리하고 #337 / #520 종료 조건을 충족한 뒤 종료한다.
