...
- eviction 보호 집합을 정의한다.
  ...
  - 초기 안정성을 위한 page 0/1
- 정책은 visible/current page 주변 window + page count soft limit + LRU 제거 순서의 혼합 정책으로 확정한다.
- `onDisappear`는 즉시 unload 트리거가 아니라 visible page 관측 정보 갱신으로만 사용한다.
- Stage 2 기본값은 최대 12개 page tree, current/visible page 주변 반경 3페이지 보호로 구현한다.
...
- access/visible metadata는 SwiftUI invalidation을 줄이기 위해 `@Published`로 노출하지 않는다.
- `loadPage(_:)`가 page tree를 만들거나 기존 page를 재사용할 때 access 정보를 갱신한다.
- `setCurrentPage(_:)`, `markPageVisible(_:)`, `markPageNotVisible(_:)` 같은 좁은 API로 current/visible 관측 상태를 갱신한다.
- `loadPage(_:)` 이후 protected page 집합을 계산하고 cache 상한 초과 시 protected page를 제외한 오래 접근하지 않은 page tree부터 제거한다.
