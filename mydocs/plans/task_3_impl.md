...
- `Vendor/rhwp`를 `edwardkim/rhwp` 기준으로 정렬한다.
- `edwardkim/rhwp`의 `devel`을 기준으로 `ios/devel`의 native viewer core 변경을 최소 반영한다.
- 포팅 대상은 render tree serde 직렬화, `build_page_render_tree`, `get_bin_data`, 관련 `Serialize` derive로 제한한다.

## 3단계: macOS bridge 연동

- `alhangeul-macos` submodule URL을 `edwardkim/rhwp`로 변경한다.
- `RustBridge`가 포팅된 core API를 사용하도록 수정한다.
- 문서에 core 추적 정책을 반영한다.
