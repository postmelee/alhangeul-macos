...
4. 질문형 프롬프트와 조건부 Before/After 표를 넣을 위치를 확정한다.
...
```bash
rg -n "대상 타스크|왜 변경했나요|무엇을 변경했나요|핵심 리뷰 포인트|후속 이슈 제안|Before|After|head_sha" \
  .github/pull_request_template.md
...
5. `task-final-report`에 `--body-file` 우선 사용과 PR 본문 짧은 작성 기준을 반영한다.
6. Git 워크플로우나 Copilot 지시 문서에 새 용어와 충돌하는 문구가 있으면 최소 문장만 보정한다.
7. Stage 3 단계 보고서를 작성한다.
...
```bash
rg -n "대상 타스크|관련 이슈|후속 이슈 제안|최대 4|최대 5행|20줄 이하|body-file|작업 문서" \
  mydocs/manual/pr_process_guide.md \
  mydocs/skills/task-final-report/SKILL.md \
  mydocs/manual/git_workflow_guide.md \
  .github/copilot-instructions.md
...
3. `관련 이슈`, `대상 타스크`, `작업 문서`, `스크린샷` 기준이 서로 충돌하지 않는지 최종 확인한다.
...
```bash
git diff --check
rg -n "대상 타스크|관련 이슈|후속 이슈 제안|핵심 리뷰 포인트|작업 문서|Before|After" \
  .github/pull_request_template.md \
  mydocs/manual/pr_process_guide.md \
  mydocs/skills/task-final-report/SKILL.md
rg -n "## 문서|Closes #" \
