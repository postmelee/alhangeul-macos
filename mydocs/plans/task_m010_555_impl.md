...
- 현재 단계: Stage 1 기술 검증 완료·보정안 검토 요청. 실제 API 263개 중 대상 6개와 공개 oEmbed 6개, 로컬 공식 임베드 렌더링 6개를 확인했다. 다음 단계 승인은 별도로 받는다.
- 방향 보정: 사용자가 자동 연결 가능 여부 설명 뒤 “진행해줘.”라고 지시하여 자동 선별·공식 임베드의 기술 검증과 계획 보정을 진행했다. 아래는 본문 복제·Git 데이터 브랜치를 제거한 보정안이며 Stage 2 진입 승인 대상으로 제시한다.
...
- `docs/index.html`: 기능 소개 다음·FAQ 이전에 ‘알한글 소식’, 최신 3개와 전체 목록 링크.
- `docs/news/index.html`: 처음 12개, ‘더 보기’로 12개씩 추가. 최신순과 마지막 버튼·포커스·상태 안내를 처리한다.
- `docs/news.js`: 같은 origin의 명시적 상대 JSON URL을 읽고 스키마 2·유효기간·원문 URL을 검증한다. 허용한 DOM 컨테이너와 원문 링크를 만들고 공식 script를 한 번만 로드한다. 동적 추가된 컨테이너의 재처리는 공식 라이브러리의 지원 경로를 Stage 3에서 확인한다. 지원이 확인되지 않으면 페이지별 navigation 등 동작 가능한 방식으로 보정한다.
- `docs/styles.css`: `.news-*` 범위에서 공식 최소 너비 320px을 고려한 반응형 배치. 본문·미디어·날짜의 내부 스타일은 공식 iframe에 맡긴다. 320px viewport에서는 불필요한 바깥 여백을 줄인다.
- 홈·업데이트 목록·문의·소식 내비게이션에 ‘소식’ 연결. 기존 다운로드·업데이트·복사 기능 보존.

유효기간이 지난 데이터는 임베드를 새로 로드하지 않고 갱신 지연과 프로필 링크를 표시한다. 정상 빈 상태, JSON 오류·재시도, script 차단·timeout, unavailable embed, JavaScript 비활성을 구분한다. cross-origin iframe 내용을 읽어 개별 글의 성공 여부를 단정하지 않으며 항상 별도의 원문 링크를 제공한다. 공식 임베드가 외부 Threads 요청을 수행한다는 점은 기존 개인정보 안내와 함께 공개 전에 반영한다.

### 검증과 종료

- 프로젝트 하위 URL에서 1440/768/390/320px, Tab/Enter/Space, JSON 0/1/3/13개, 더 보기, script 중복·실패·재시도·만료 상태를 확인한다.
