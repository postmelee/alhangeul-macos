...
## Stage 3.1 — 실제 Pages 배포 gate 누락 보정

#550 병합 후 workflow 34930122727은 prepare 성공·deploy 생략이었다. 기존 v0.2.0 헤더의 미공개 v0.2.1 DMG 링크가 asset gate를 막았다. 같은 유형의 v0.1.11 latest URL도 고정 버전으로 정렬한다. 과거 페이지의 다운로드는 해당 공개 버전으로 고정하고, 최신 버전 이동은 기존 배너를 사용한다. 전체 링크가 가리키는 공개 자산을 API로 대조한 뒤 한 개 후속 main PR에 보정·증거를 묶어 CI·실제 Pages 배포 후 #551 인계에 포함한다.
