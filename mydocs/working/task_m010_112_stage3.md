...
```bash
for pattern in "대상 타스크" "관련 이슈" "후속 이슈 제안" "최대 4" "최대 5행" "20줄 이하" "body-file" "작업 문서" "단계 보고서"; do
...
for pattern in "Open PR" "body-file" "작업 문서" "단계 보고서" "commit SHA"; do
  rg -n "$pattern" mydocs/manual/git_workflow_guide.md
done
for pattern in "direct target task" "contextual related issues" "report and commit" "verification"; do
  rg -n "$pattern" .github/copilot-instructions.md
done
```

결과: 통과. 파일별 필수 패턴을 각각 확인해 `대상 타스크`, 맥락용 `관련 이슈`, `후속 이슈 제안`, 짧게 강제하는 기준, `--body-file`, 작업 문서, Stage report+commit 링크 기준을 확인했다. Copilot 지시는 같은 의미를 영어 문구 기준으로 확인했다.
