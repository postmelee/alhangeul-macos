...
작업지시자 확인에서 홈 화면 섹션 2와 섹션 3도 섹션 1과 같은 fade-in delay와 시간차를 갖도록 요청되었다. 섹션 2인 `.app-intro-section`과 섹션 3인 `.features-sticky`의 `data-reveal-gap`을 섹션 1과 같은 `180`으로 맞췄다. 또한 기존 스크립트는 페이지 로드 900ms 뒤 모든 reveal group을 강제로 열어 이후 섹션이 스크롤 진입 전에 이미 보이는 문제가 있었으므로, fallback은 현재 viewport에 걸친 그룹에만 적용되도록 바꿨다. 변경된 `script.js`와 `styles.css`가 브라우저 캐시에 남지 않도록 HTML의 asset query도 `20260510-reveal`로 갱신했다.

## 추가 보정 10

작업지시자 확인에서 섹션 3의 첫 영상인 `Finder에서 썸네일로 찾기`가 reveal 이전부터 재생되어, 화면에 나타날 때 이미 영상이 진행 중인 문제가 있었다. `setupFeatureShowcase()`의 초기 active 상태 설정은 유지하되 `play: false`로 첫 영상을 정지 상태에 두고, `.features-sticky` reveal group이 열린 뒤 `.feature-showcase` reveal item의 transition이 끝나면 active 영상을 처음부터 재생하도록 했다. 이를 위해 reveal group을 여는 공통 함수에서 섹션 3 reveal을 감지하고, transition duration과 delay를 계산한 fallback timer도 함께 둬 transitionend 누락 상황에도 재생이 시작되도록 했다. 변경된 `script.js`가 브라우저 캐시에 남지 않도록 홈 HTML의 script query를 `20260510-reveal-video`로 갱신했다.
...
```bash
curl -s http://127.0.0.1:8766/ | rg -n "data-reveal-gap=\"180\"|script\\.js\\?v=20260510-reveal|styles\\.css\\?v=20260510-reveal"
curl -s 'http://127.0.0.1:8766/script.js?v=20260510-reveal' | rg -n "setTimeout|isVisible|revealAll|is-revealed"
```

결과: 홈 HTML에서 섹션 1, 2, 3의 reveal gap이 `180`으로 서빙되고, `script.js`의 fallback이 현재 보이는 그룹만 reveal하도록 변경된 것을 확인했다.

Browser quick check:

- URL: `http://127.0.0.1:8766/?qa=reveal-20260510`
- viewport: `1280 x 720`
- page title: `알한글 - Mac용 HWP/HWPX 뷰어`
- console error/warn: 없음
- 초기 상태: `.app-intro-section`, `.features-sticky`는 첫 화면에서 `is-revealed`가 붙지 않음
- 섹션 2 진입 후: `.app-intro-section is-revealed`, reveal item delay `0ms`, `180ms`, `360ms`
- 섹션 3 진입 후: `.features-sticky is-revealed`, reveal item delay `0ms`, `180ms`, `360ms`

```bash
curl -s http://127.0.0.1:8766/ | rg -n "script\\.js\\?v=20260510-reveal-video"
curl -s 'http://127.0.0.1:8766/script.js?v=20260510-reveal-video' | rg -n "hasStartedFeaturePlayback|startFeaturePlaybackAfterReveal|play: false|setupFeatureShowcase\\(\\);|setupRevealAnimations\\(\\);"
```

결과: 홈 HTML이 `20260510-reveal-video` script query를 서빙하고, 최신 스크립트에 초기 feature video 정지 상태와 reveal 이후 재생 제어가 포함된 것을 확인했다.

Browser quick check:

- URL: `http://127.0.0.1:8766/?qa=reveal-video-20260510`
- viewport: `1280 x 720`
- page title: `알한글 - Mac용 HWP/HWPX 뷰어`
- console error/warn: 없음
- 초기 상태: `.features-sticky`는 첫 화면에서 `is-revealed`가 붙지 않음
- 섹션 3 진입 후: `.features-sticky is-revealed`, `#feature-video-0`은 `feature-video is-active`, `.feature-showcase` reveal delay는 `360ms`
