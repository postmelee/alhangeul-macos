...
- `README.md`: 432 lines
  ...
  - 이전 README의 로드맵 흐름 표현은 복구하되, 미래 버전별 미구현 기능 체크리스트는 복구하지 않음
  - `v0.1.x 구현 범위`를 추가해 현재 공개 릴리즈 라인에서 이미 제공하는 기능만 체크리스트로 표시
  - `Features` 아래의 `v0.1 Known Limitations`를 `v0.1.x 구현 범위`의 `현재 제한 사항`으로 이동
  - 최신 공개 릴리즈는 실제 공개 상태가 확인된 `v0.1.0`으로 유지
  - `v0.1.1`은 public release 완료 전 patch release 후보로만 표시
- `mydocs/tech/product_roadmap_notes.md`: 77 lines
  - README에는 큰 이정표와 현재 구현 기능만 두고, 미래 마일스톤 후보는 제품 로드맵 메모에서 관리한다는 기준 추가
...
- 구현된 기능 체크리스트는 미래 마일스톤 약속이 아니라 현재 공개 릴리즈 라인의 제공 범위로 한정했다.
- 릴리즈 라인의 제한 사항은 `Features` 하위가 아니라 `v0.1.x 구현 범위` 하위로 옮겨, 기능 설명과 제공 범위의 경계를 분리했다.
- 미래 마일스톤 후보 기능과 구현 순서의 관리 위치는 `mydocs/tech/product_roadmap_notes.md`에 명시했다.
...
- README에서 미래 기능 후보는 `product_roadmap_notes.md`로 넘기고, 현재 구현 체크리스트는 `v0.1.x 구현 범위`에 한정한 것을 확인
- `product_roadmap_notes.md`에 README 반영 기준이 추가된 것을 확인
...
```text
     ...
     110 docs/updates/index.html
      ...
      77 mydocs/tech/product_roadmap_notes.md
     716 total
```
