...
- `Sources/` 아래에 `HostApp`, `QLExtension`, `ThumbnailExtension` 제품 타깃을 먼저 노출했다.
...
- 사용자 피드백에 따라 비교 대상 프로젝트를 README에서 직접 언급하는 문장은 제거했다.
...
```text
$ rg --line-number 'Sources/|HostApp/|QLExtension/|ThumbnailExtension/|Shared/|RhwpCoreBridge/|RustBridge/|Frameworks/|project.yml|rhwp-core.lock|samples/' README.md
...
- README에서는 비교 대상 프로젝트를 직접 언급하지 않도록 보정했다. 상세 비교와 active 문서 전체 검색 gate는 Stage 5에서 다시 다루는 것이 맞다.
