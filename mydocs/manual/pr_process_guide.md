...
내부 task PR은 `local/task{번호}`에서 작업한 내용을 `publish/task{번호}`로 push한 뒤 작업 범위에 맞는 통합 브랜치 대상으로 생성한다. 첫 공개 배포, WKWebView-backed viewer/editor, Finder/Quick Look, PDF/공유/저장, Spotlight/mdimporter, 변환, 배포, 문서, Skia 공통 기반은 `devel`, HostApp native macOS shell, Skia viewport, Swift 편집 UI/오버레이는 `native-viewer-editor`를 쓴다.
