...
| `run_rust_verify` | Rust bridge/core lock 검증 또는 macOS build용 generated `Rhwp.xcframework` 준비 필요 | `run_macos_build=true`인 변경, `RustBridge/**`, `rhwp-core.lock`, `Frameworks/**`, core 관련 scripts |
...
`Frameworks/Rhwp.xcframework`는 git에 commit하지 않는 generated artifact이므로, GitHub Actions fresh runner의 macOS validation은 HostApp build 전에 항상 Rust bridge artifact를 재생성하고 `rhwp-core.lock`과 대조한다.
