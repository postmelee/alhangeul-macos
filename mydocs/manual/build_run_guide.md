...
현재 submodule 단계의 core source repository는 `edwardkim/rhwp`이며, submodule URL과 lock provenance는 `.gitmodules`와 `rhwp-core.lock`을 기준으로 확인한다.
...
현재 기본 개발 흐름은 `Vendor/rhwp` submodule과 `rhwp-core.lock`을 함께 검증한다. 후속 Issue #30에서 submodule을 제거하면 `RustBridge/Cargo.toml`은 `edwardkim/rhwp` git dependency를 사용한다.
...
현재 submodule 단계에서 `rhwp-core.lock`의 `rhwp_repo`는 provenance 기록 항목이며, repository 기준 정합성은 `.gitmodules`와 함께 확인한다. git dependency 전환 후에는 `RustBridge/Cargo.toml`, `RustBridge/Cargo.lock`, `rhwp-core.lock`의 repo/ref/commit 정합성을 함께 확인한다.
...
git dependency 전환 후에는 `Vendor/rhwp` 준비 단계가 사라지고, 첫 Rust bridge build에서 Cargo가 `edwardkim/rhwp`를 fetch한다. 네트워크 실패는 core compatibility 실패가 아니라 dependency fetch 실패로 분리해 기록한다.
