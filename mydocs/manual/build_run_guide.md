...
Codex sandbox 안에서 `qlmanage` 기반 Core SVG raster diff가 다음 오류로 실패할 수 있다.

```text
sandbox initialization failed: invalid data type of path filter; expected pattern, got boolean
```

이 경우 rhwp core SVG 생성 실패나 Swift native renderer 회귀가 아니라 macOS Quick Look/`qlmanage`의 sandbox 초기화 실패로 분리한다. 일반 macOS Terminal에서 같은 SVG에 대해 다음 명령이 성공하면 환경성 제한으로 판단한다.

```bash
qlmanage -t -x -s 1200 -o /tmp/rhwp-ql-test path/to/page-core.svg
```

단계 보고서에는 `DiffReason: qlmanage rasterize failed`를 환경성 제한으로 기록하고, native PNG 생성 여부와 smoke summary를 별도로 판단한다. Codex 안에서 안정적인 pixel diff가 반드시 필요하면 `qlmanage` 대신 별도 SVG rasterizer fallback을 추가하는 작업으로 분리한다.
