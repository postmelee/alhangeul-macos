...
다음 항목은 현재 release script, Cask, plist에 반영된 운영 기준이다. 특정 릴리즈 시점의 결정 기록은 [`mydocs/release/`](../release/)의 릴리즈별 문서에 남긴다.
...
다음 항목은 각 public release 시점에 작업지시자 결정이 필요하다.
...
5. 수정 PR을 출시 대상 통합 브랜치로 merge한 뒤 새 릴리스 후보를 만든다. 현재 WebView-backed release line 기준은 `devel-webview`이며, native renderer 장기 브랜치에도 필요한 수정은 별도 PR 또는 cherry-pick으로 `devel`에 후속 반영한다.
