...
v0.1.0 public release 이후 GitHub Pages source는 `main` `/docs`를 기준으로 운영한다.

- repository variable `ALHANGEUL_PAGES_BRANCH`는 GitHub Pages source branch와 같아야 한다.
- 현재 기준 값은 `main`이다.
- `Release Publish DMG` workflow가 `docs/appcast.xml`을 갱신해도 repository Pages source가 다른 branch를 보고 있으면 public appcast URL은 갱신되지 않는다.
- release 후에는 저장소의 `docs/appcast.xml`만 확인하지 말고 반드시 `https://postmelee.github.io/alhangeul-macos/appcast.xml` 응답을 확인한다.
- Pages source를 바꾸거나 수동 build를 요청했다면 `pages-build-deployment` run이 새 `main` commit 기준으로 success인지 확인한다.
...
- workflow는 `ALHANGEUL_PAGES_BRANCH` repository variable을 Pages source branch로 사용한다. 값이 없으면 `devel-webview`를 기본값으로 사용하므로, 현재 Pages 기준인 `main`을 명시해 둔다.
...
- 설치본에서 `알한글 > 업데이트 확인...`을 사용자가 직접 실행했을 때 Sparkle UI가 표시되는가

첫 실행 직후 "최신 버전입니다" 창이 자동으로 뜨지 않는 것은 실패로 보지 않는다. `SUEnableAutomaticChecks=true`는 자동 확인 스케줄을 허용하는 설정이지, 매 실행마다 사용자-facing 최신 버전 창을 띄우는 기준이 아니다. 수동 smoke의 기대 결과는 메뉴 항목을 직접 눌렀을 때 최신 버전 또는 업데이트 가능 창이 표시되는 것이다.
...
- CI에서 필요한 native tool(`cbindgen` 등)은 workflow 안에서 설치해야 한다. 로컬에 설치되어 있다는 이유로 CI 통과를 가정하지 않는다.
- GitHub Actions의 Node.js deprecation warning은 release 실패 원인이 아닐 수 있지만, 다음 release 전에 별도 maintenance issue로 정리한다.

### 서명과 공증

- Xcode archive/build 결과가 Developer ID identity를 사용하더라도 nested Sparkle code가 timestamp와 runtime 조건을 모두 만족한다고 가정하지 않는다.
- Sparkle `Downloader.xpc`, `Installer.xpc`, `Updater.app`, `Autoupdate`, `Sparkle.framework`는 release script에서 Developer ID, secure timestamp, runtime, entitlements 상태를 명시적으로 검증한다.
- Host app과 appex에는 release entitlements를 별도로 준비하고 `get-task-allow`가 남아 있지 않은지 확인한다.
- notarization 실패 시 `notarytool log`를 workflow log에 출력해 다음 실행 전에 원인을 좁힌다.

### Git tag와 appcast commit

- release tag는 appcast bot commit 이전의 release source commit을 가리킬 수 있다. v0.1.0에서는 `v0.1.0` tag가 release workflow head를 가리키고, 이후 GitHub Actions bot이 `docs/appcast.xml`을 `main`에 추가 커밋했다.
- appcast bot commit이 생겼다고 같은 release tag를 새 commit으로 다시 옮기지 않는다.
- 이미 public GitHub Release, appcast, Cask SHA가 공개된 버전은 같은 tag/version으로 덮어쓰지 않는다. 수정 배포는 `0.1.1`, `0.1.2`처럼 patch version을 올린다.

### Pages와 appcast

- `ALHANGEUL_PAGES_BRANCH`와 GitHub Pages source branch가 다르면 workflow는 성공해도 public appcast URL이 stale 상태일 수 있다.
- release 직후에는 `gh api repos/postmelee/alhangeul-macos/pages`로 source branch/path를 확인한다.
- public appcast 검증은 `curl`로 실제 `https://postmelee.github.io/alhangeul-macos/appcast.xml`을 내려받아 `xmllint`와 `sparkle:version`, `sparkle:shortVersionString`, `sparkle:edSignature`, enclosure URL을 확인한다.

### 설치본 smoke

- 오래된 로컬 앱(`/Users/melee/Applications/AlhangeulMac.app` 등)이나 실행 중인 이전 프로세스가 있으면 `/Applications/Alhangeul.app` smoke와 혼동될 수 있다. public DMG 검증 전에는 이전 앱과 프로세스를 정리한다.
- macOS 최신 버전에서는 `lsregister -kill`이 제거되어 있다. Launch Services 갱신은 `lsregister -r -domain local -domain system -domain user`로 수행한다.
- 설치본 smoke는 최소한 앱 실행, HWP/HWPX 열기, Sparkle 수동 업데이트 확인, 창 확대/resize, Quick Look/Thumbnail 확인을 포함한다.
- Sparkle은 첫 실행 때 최신 버전 창을 자동으로 띄우지 않을 수 있다. 메뉴에서 `알한글 > 업데이트 확인...`을 직접 눌러 확인한다.
- v0.1.0 smoke에서 창 상단 헤더 더블 클릭 확대 시 WebView runtime error가 발견되어 #183으로 분리했다. resize/zoom 계열 동작은 다음 patch release gate에 포함한다.

### Homebrew Cask

- repository 안의 `Casks/alhangeul-macos.rb`는 release 기준 source일 뿐, Homebrew public tap context가 아니다.
- `ruby -c`와 `scripts/update-cask-sha256.sh --dry-run`은 repository-local 검증으로 사용한다.
- `brew audit --cask Casks/alhangeul-macos.rb` 같은 raw path audit은 Homebrew 설정에 따라 거부될 수 있다. 최종 audit은 tap에 반영한 뒤 token 기준으로 수행한다.
...
- [ ] repository variable `ALHANGEUL_PAGES_BRANCH=main` 확인
...
- [ ] public appcast URL을 내려받아 XML, `sparkle:version`, `sparkle:shortVersionString`, `sparkle:edSignature`, enclosure URL 확인
- [ ] Pages 다운로드 버튼과 appcast URL이 public DMG asset을 가리키는지 확인
- [ ] `scripts/update-cask-sha256.sh`로 Cask version/sha256 갱신
- [ ] public DMG 설치본에서 앱 실행, 문서 열기, 창 확대/resize, Sparkle 수동 업데이트 확인 smoke 완료
- [ ] Homebrew tap 대상 확정
