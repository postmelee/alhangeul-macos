...
### `working/`과 `report/` 위치를 혼동했을 때

단계별 완료보고서(`_stage{N}.md`)는 `mydocs/working/`에 둔다. 최종 결과보고서(`_report.md`)는 `mydocs/report/`에 둔다. 잘못된 위치에 만들었다면 커밋 전에는 올바른 폴더로 이동하고, 커밋 후 발견하면 다음 단계 보고서 또는 최종 보고서에 보정 사실을 기록한다. 폴더 기준은 [`document_structure_guide.md`](document_structure_guide.md)를 따른다.
