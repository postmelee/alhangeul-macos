...
Architecture rules:
- `Sources/RhwpCoreBridge` must not import or depend on AppKit/UIKit and must not own UI state. It should stay limited to FFI wrappers, render tree decoding, current CoreGraphics/CoreText fallback/diagnostic rendering, and explicit Rust/rhwp Skia bridge contracts.
