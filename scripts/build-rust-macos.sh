...
if [ ! -f "$RHWP_ROOT/cbindgen.toml" ]; then
  echo "ERROR: upstream rhwp devel does not currently expose cbindgen.toml for the native viewer ABI." >&2
  echo "This project must add a local Rust bridge crate or wait for a public native bridge API in rhwp core." >&2
  exit 1
fi
if ! grep -q 'staticlib' "$RHWP_ROOT/Cargo.toml"; then
  echo "ERROR: upstream rhwp devel is not currently configured to build a staticlib." >&2
  echo "This project must build a separate bridge staticlib that depends on Vendor/rhwp." >&2
  exit 1
fi
...
xcrun lipo -create \
  "$RHWP_ROOT/target/aarch64-apple-darwin/release/librhwp.a" \
  "$RHWP_ROOT/target/x86_64-apple-darwin/release/librhwp.a" \
  -output "$OUT/universal/librhwp.a"
xcrun lipo -info "$OUT/universal/librhwp.a"

echo "[3/4] cbindgen header check..."
cbindgen --quiet --config "$RHWP_ROOT/cbindgen.toml" --crate rhwp \
  --output "$GENERATED_H" "$RHWP_ROOT"
