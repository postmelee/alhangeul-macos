...
```
v0.1(WebView로 먼저 배포한다) → v0.2(Mac 통합을 넓힌다) → v0.3(변환하고 자동화한다) → v0.4(믿고 설치한다) → v0.5(Swift native viewer를 만든다) → v0.6(Swift native editor 기반을 만든다) → v1.0(native로 안전하게 고친다) → v2.0(AI가 다룬다)
```

| 버전 | 단계 | 사용자에게 보이는 변화 | 주요 범위 |
|------|------|------------------------|-----------|
| `v0.1` | WebView로 먼저 배포한다 | Finder와 Quick Look에서 HWP/HWPX가 보이고, 앱에서는 `rhwp-studio` 기반 viewer/editor로 열기, 찾기, 복사, 저장, 공유, PDF 내보내기를 할 수 있습니다. | Quick Look preview, Finder thumbnail, WKWebView HostApp, 저장/다른 이름 저장, PDF export, share, Finder reveal, 첫 배포 |
| `v0.2` | Mac 통합을 넓힌다 | 앱 화면 밖에서도 문서 정보와 내용을 더 잘 다룰 수 있게 합니다. | Quick Look lazy preview 검토, 문서 정보/본문 추출, Spotlight/mdimporter 검토, Mac 서비스 연동 기반 |
| `v0.3` | 변환하고 자동화한다 | 여러 HWP/HWPX 문서를 Finder, CLI, Shortcuts 흐름에서 변환할 수 있습니다. | Text/Markdown/blocks JSON/HWPX 변환, batch 변환, Quick Action, CLI, Shortcuts |
| `v0.4` | 믿고 설치한다 | 일반 사용자가 설치해도 되는 안정적인 Mac 앱으로 다듬습니다. | signed/notarized 배포, 업데이트, crash-safe opening, 접근성, 호환성 리포트 |
| `v0.5` | Swift native viewer를 만든다 | WebView에 의존하지 않는 Mac native 문서 보기 경험을 기본 경로로 끌어올립니다. | native rendering parity, native zoom/scroll/page cache, page sidebar, native search/copy |
| `v0.6` | Swift native editor 기반을 만든다 | native viewer 위에 caret, selection, IME, overlay를 올려 편집 가능한 구조를 만듭니다. | caret/selection/IME, ruler/margin/table/object overlay, dirty state, undo/redo 기반 |
| `v1.0` | native로 안전하게 고친다 | 문서 손상을 피하면서 자주 필요한 작은 수정을 Swift native editor에서 지원합니다. | 제한된 텍스트/문단/표 편집, autosave, 복사본 저장, round-trip 검증 |
...
> Finder에서 HWP/HWPX 파일이 낯선 회색 아이콘이 아니라 "내용이 보이는 문서"가 되고, 앱에서는 `rhwp-studio` viewer/editor를 WKWebView로 품어 첫 공개 배포가 가능한 제품 표면을 만듭니다.
...
- [x] Finder 또는 다른 앱의 열기 요청으로 새 viewer window 열기
- [x] Finder에서 viewer 영역으로 끌어와서 열기
- [x] `Command+O/S/Shift+S/P`와 파일 메뉴의 열기, 저장, 다른 이름으로 저장, 인쇄 연결
- [x] HWP 문서 저장과 다른 이름으로 저장
- [x] PDF로 내보내기
- [x] macOS 공유 sheet로 현재 문서 공유
...
- [ ] 페이지별 텍스트 추출
- [ ] 표 텍스트 추출
- [ ] Quick Look visible-page lazy preview 가능성 검증
- [ ] View-based Quick Look preview 전환 검토
...
- [ ] PageLayerTree native bridge와 FFI 공존 경로
- [ ] native viewport, DPR-aware rendering, page pooling
- [ ] native trackpad pinch zoom, 확대 중심점 유지, scroll offset 보정
- [ ] 페이지 썸네일 sidebar와 페이지 번호 이동
- [ ] native search/copy/selection
...
- [ ] caret, selection, IME composition overlay
- [ ] ruler, margin guide, table/object overlay
- [ ] native editor command routing
- [ ] dirty state와 autosave 정책 설계
- [ ] undo/redo 기반
...
- [ ] 복사본 저장, round-trip test, 손상 방지 정책 고도화
- [ ] HWPX 저장 지원
