...
- `edwardkim/rhwp`의 `devel`을 `Vendor/rhwp` submodule로 연결
...
현재 submodule 단계에서는 `rhwp` core를 `edwardkim/rhwp`의 `devel` 브랜치 기준으로 고정합니다. 현재 고정 commit과 Rust bridge 산출물 provenance는 [rhwp-core.lock](rhwp-core.lock)에 기록합니다. 후속 dependency 전환 단계에서는 `edwardkim/rhwp` 릴리즈 태그와 resolved commit을 함께 고정합니다.
...
Vendor/
└── rhwp/                     # edwardkim/rhwp devel submodule
