...
- 코어 브릿지(RhwpCoreBridge)
  ...
  - core 안정 기준을 floating branch가 아니라 release tag + resolved commit으로 다시 고정했고, 현재 `edwardkim/rhwp` 최신 release `v0.7.3`에는 필요한 bridge API가 없어 즉시 전환할 수 없음을 lock과 운영 기준에 반영했습니다.
...
앱의 Stable 안정 기준은 `edwardkim/rhwp`의 release tag와 resolved commit을 함께 고정하는 것입니다. 필요한 bridge API가 아직 upstream release tag에 포함되지 않은 경우 Demo/Preview 경로로 특정 commit을 `rev`에 고정합니다. 두 경우 모두 `Cargo.lock`, `rhwp-core.lock`, Rust bridge 산출물 hash/size가 함께 맞아야 합니다.
...
└── update-rhwp-core.sh       # rhwp git dependency와 rhwp-core.lock 갱신
```
