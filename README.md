...
`v0.1.2`는 Sparkle 업데이트 후 Finder thumbnail stale cache를 줄이기 위한 첫 실행 maintenance, 앱 소유 UTI `com.postmelee.alhangeul.*`와 Hancom 계열 UTI 지원, Web viewer runtime 오류의 non-blocking banner 표시, About 창의 bundled `rhwp` provenance 표시, upstream `rhwp v0.7.11` core/studio 반영을 포함하는 release candidate입니다. public 배포는 제품 개발 브랜치의 검증 commit을 `main`에 반영하고 `v0.1.2` tag를 만든 뒤 별도 승인으로 진행합니다.
...
- 제품 기능과 배포 후보 작업은 `devel`, Swift native viewer/editor 실험 작업은 `native-viewer-editor` 기준으로 분리
...
- PR base는 작업 범위에 따라 다릅니다. 첫 공개 배포, WKWebView-backed viewer/editor, Finder/Quick Look, PDF/공유/저장, Mac 통합/변환, 배포, 문서 작업은 `devel`로 보내고, Swift native viewer/editor 관련 기여는 `native-viewer-editor`로 보냅니다. `main`은 릴리즈 브랜치입니다.
