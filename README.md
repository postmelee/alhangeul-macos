...
공개 배포 기준은 Developer ID로 서명하고 Apple notarization을 통과한 DMG입니다. GitHub Release에는 `alhangeul-macos-<version>.dmg`와 checksum을 함께 공개합니다. Homebrew Cask는 같은 notarized DMG와 고정 `sha256`을 기준으로 tap 검증이 끝난 뒤 안내합니다.
