...
현재 이 저장소는 `edwardkim/rhwp` 코어 엔진을 `Vendor/rhwp` git submodule로 고정해 사용합니다. 앱, Quick Look 확장, Thumbnail 확장, Swift bridge, macOS UX는 이 저장소가 소유합니다.
...
- `edwardkim/rhwp` core를 `Vendor/rhwp` submodule로 연결하고, 릴리즈 태그 전환 전까지 필요한 bridge API가 포함된 commit을 lock으로 고정
...
- `edwardkim/rhwp`를 `Vendor/rhwp` submodule path dependency로 사용하는 `RustBridge` crate
...
앱의 안정 기준은 `edwardkim/rhwp`의 release tag와 resolved commit을 함께 고정하는 것입니다. 현재 submodule 단계의 `rhwp-core.lock`은 release tag 전환 대기 상태를 명시하고, 필요한 bridge API가 포함된 commit과 Rust bridge 산출물 provenance를 기록합니다. 후속 dependency 전환 단계에서는 `edwardkim/rhwp` 릴리즈 태그와 resolved commit을 실제 dependency 기준으로 고정합니다.
...
Vendor/
└── rhwp/                     # edwardkim/rhwp core submodule
