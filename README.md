...
현재 이 저장소는 `edwardkim/rhwp` 코어 엔진을 `RustBridge/Cargo.toml`의 git dependency로 고정해 사용합니다. 앱, Quick Look 확장, Thumbnail 확장, Swift bridge, macOS UX는 이 저장소가 소유합니다.

v0.1.0의 현재 배포 목표는 Demo/Preview release입니다. 필요한 bridge API가 포함된 resolved commit을 `rev`로 고정하고, Stable release는 upstream release tag가 같은 API를 포함할 때 별도 승격합니다.
...
- `edwardkim/rhwp` core를 `RustBridge` git dependency로 연결하고, 필요한 bridge API가 포함된 commit을 Demo/Preview 기준으로 lock에 고정
...
<details>
<summary>최근 변경 (v0.1.0, 2026-04-26)</summary>
...
- 코어 브릿지(RhwpCoreBridge)
  - `rhwp-core.lock` v2를 도입해 `librhwp.a`와 generated header의 `sha256`/`size`를 고정하고, `Cargo.lock`의 `rhwp` git dependency commit과 함께 provenance를 검증하도록 강화했습니다.
  - `build-rust-macos.sh --update-lock/--verify-lock`과 package 전 lock verify를 추가해 bridge 산출물 불일치를 조기에 차단하도록 했습니다.
  - core 안정 기준을 floating branch가 아니라 release tag + resolved commit으로 다시 고정했고, 현재 `edwardkim/rhwp` 최신 release `v0.7.3`에는 필요한 bridge API가 없어 Stable release tag 승격이 blocked 상태임을 lock과 운영 기준에 반영했습니다.
...
- `edwardkim/rhwp`를 git `rev` dependency로 사용하는 `RustBridge` crate
...
처음 프로젝트에 참여하는 개발자는 이 README의 [Project Structure](#project-structure)를 먼저 보고, 세부 경계는 [아키텍처 문서](mydocs/tech/project_architecture.md), 실행 절차는 [빌드 및 실행 가이드](mydocs/manual/build_run_guide.md)를 확인하세요. 실제 빌드는 Rust bridge 산출물을 만든 뒤 Xcode project를 생성하고 HostApp을 빌드하는 순서입니다.
...
Finder Quick Look/Thumbnail extension 등록 핵심 흐름:
...
`lsregister` 갱신, Quick Look 캐시 초기화(`qlmanage -r`), thumbnail/preview smoke test(`qlmanage -t -x`, `qlmanage -p`) 등 전체 절차와 반복 시행착오 방지 규칙은 [빌드 및 실행 가이드](mydocs/manual/build_run_guide.md)의 "Finder 통합 확인" 섹션을 참고하세요.
...
현재 v0.1.0은 Demo/Preview release를 목표로 하므로 `--channel demo --rev`가 기본 경로입니다. Stable 승격은 upstream release tag가 필요한 bridge API를 포함할 때 별도 진행하며, 현재 `v0.7.3`은 `build_page_render_tree`, `get_bin_data`가 없어 blocked 상태입니다.

채널별 dependency/lock 기준과 compatibility gate 상세는 [`core_release_compatibility.md`](mydocs/tech/core_release_compatibility.md)를 참조하세요.
...
타깃 간 소유 경계, 공통 Swift 계층, Rust bridge, 런타임 데이터 흐름은 [`project_architecture.md`](mydocs/tech/project_architecture.md)를 참조하세요.

## AI 페어 프로그래밍으로 개발합니다

> 출처: 이 섹션의 문제의식과 개발 방법론 설명은 [`edwardkim/rhwp` README.md](https://github.com/edwardkim/rhwp/blob/main/README.md)의 "AI 페어 프로그래밍으로 개발합니다" 섹션을 바탕으로 합니다. alhangeul-macos에서는 같은 절차를 Claude Code와 OpenAI Codex에 함께 적용합니다.
