...
- [ ] `edwardkim/rhwp` `devel-webview` 브랜치의 `rhwp-studio`를 WKWebView viewer로 로드
- [ ] 앱 sandbox 안에서 로컬 HWP/HWPX 파일을 WKWebView viewer에 안전하게 전달
- [ ] WKWebView 기반 viewer의 기본 스크롤, 확대/축소, 페이지 이동, 오류 상태 처리
- [ ] 첫 MVP 배포 산출물 구성
...
- `edwardkim/rhwp` `devel-webview` 브랜치의 `rhwp-studio` viewer 통합
- HWP/HWPX 파일 열기
